...
router.get('/fill_database', function (req, res) {
     ...
     */
    let u;
    User.find({ "where": { "id": 3 } }).then(function (user) { u = user; }).catch(function (err) { console.log('nok' + err); });

    Product.create({ name: "Doctor Who", amount: 3, price: 119.99, barecode: "001275468953", user: User.find({ "where": { "id": 3 } }) }).then(function (p) {
